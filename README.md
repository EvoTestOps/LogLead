...
* Custom Unsupervised (4): [Out-of-Vocabulary Detector](https://github.com/EvoTestOps/LogLead/blob/main/loglead/OOV_detector.py) counts amount words or character n-grams that are novel in test set. [Rarity Detector](https://github.com/EvoTestOps/LogLead/blob/main/loglead/rarity_detector.py), scores seen words or character n-grams based on their rarity in training set. See our public [preprint](https://arxiv.org/abs/2312.01934) for more details. [Next Event Prediction](https://github.com/EvoTestOps/LogLead/blob/main/loglead/sequence_anomaly_detectors.py) scores a sequence with an n-gram model of event order, and [Lookahead Pairs](https://github.com/EvoTestOps/LogLead/blob/main/loglead/sequence_anomaly_detectors.py) flags event pairs never seen within a window in the training set. Both need parsed event sequences.
