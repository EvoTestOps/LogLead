"""Long-lived corpus sessions: load, mask, and parse a corpus exactly once.

This is the point of the MCP server. Loading a corpus is cheap; masking it and
...
* opening a corpus reads it, masks it, and pre-parses whatever was asked for;
...
@dataclass
class Session:
    """One loaded corpus, plus every enhanced column derived from it so far."""
    ...
    @property
    def runs(self):
        return self.df.select("run").unique().sort("run").to_series().to_list()
    ...
    def summary(self):
        return {
            ...
            "n_runs": self.df.select("run").n_unique(),
    ...
    def ensure_content(self, mask, content_format):
        ...
        if mask and not self.masked:
            raise ValueError(
                ...
                "no 'e_message_normalized' column. Re-open the corpus with mask=True, "
                "or pass mask=False to this analysis."
            )

        field = "e_message_normalized" if mask else "m_message"
        target = corpus.content_column(mask, content_format)
        derived = corpus.derived_columns(content_format)
        ...
        df, resolved_field = corpus.prepare_content(self.df, mask, content_format)
...
class SessionStore:
    """Open sessions, backed by a parquet cache keyed on corpus + preprocessing."""
    ...
    def _cache_key(self, root, filename_pattern, mask_pattern, file_name_normalizer,
                   min_file_size):
        n_files, total_bytes, max_mtime = corpus.count_corpus_files(
        ...
        # Corpus fingerprint is cheap (stat only) but catches added, removed,
    ...
    def open(self, path, filename_pattern="*.log", mask=True,
             ...
             refresh=False):
        """Load a corpus into a session, reusing the parquet cache when possible.

        :param mask: run :meth:`EventLogEnhancer.normalize` at open time.
            Required by every ``mask=True`` analysis and by all pre-parsing.
        :param mask_pattern: name from :data:`loglead.analysis.masking.PATTERNS`.
        ...
        if file_name_normalizer not in corpus.FILE_NAME_NORMALIZERS:
            raise ValueError(
                f"Unknown file_name_normalizer {file_name_normalizer!r}. "
                f"Valid options: {sorted(corpus.FILE_NAME_NORMALIZERS)}"
            )
        ...
        if cache_hit:
            ...
            df, _ = corpus.read_folders(root, filename_pattern, min_file_size)
            if mask:
                df = EventLogEnhancer(df).normalize(
                    regexs=masking.get_pattern(mask_pattern)
                )
            df = corpus.normalize_file_names(df, file_name_normalizer)
    ...
    def get(self, session_id):
        try:
            ...
            raise ValueError(
                ...
                + (f"Open sessions: {known}" if known else "Call open_corpus first.")
            ) from None
        session.touch()
        return session
